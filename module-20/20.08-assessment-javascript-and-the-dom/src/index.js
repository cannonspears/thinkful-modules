...
function render(contacts) {
  const contactsHtml = contacts.map((contact) => {
    return contactHtml = renderContact(contact)
  })
...
function filterHandler() {
  const cityOptions = document.querySelector("#filterOptions")
...
function loadCities(contacts = []) {
  const arrayOfCitiesOnly = contacts.map((contact) => {
  ...
  const arrayOfOptionsHtml = uniqueCities.map((city) => {
    return `<option value="${city}">${city}</option>`
  })

  const selectTag = document.querySelector("#filterOptions")
  selectTag.innerHTML = ""
  selectTag.innerHTML += `<option value="0">-- Select a city --</option>`
  selectTag.innerHTML += arrayOfOptionsHtml
}
...
function deleteContact(id) {
  const indexOfContact = window.contacts.findIndex(contact=>contact.id === Number(id))
  if (indexOfContact >= 0) {
    window.contacts.splice(indexOfContact, 1)
  }
}
